...
**Monitor controller :**
- `crd/controllers/monitor/monitor_controller.go` : runtime port discovery (defined in the architecture above as monitor listener)
**Debug Container :** 
...
For each RunningPOD in TargetedNameSpace:
		a. AddPodEvent() 
		b. if !SystemPOD : 
			installEphemeralContainers() #injects ephemeral container debugger
		# Generate nmap commands for the Pod
		c. BuildTargetedCommands() && BuildCommandsFromPodSelector()
		d. store commands in event-storage
...
Every 10 sec do: 
	...
	Discover not declared ports in pod spec
	build new high priority queue
```
***7 - Results fetched by the user : `kubectl port-forward deployment/kubesonde-controller-manager 2709 | curl localhost:2709/probes > results.json` *** 
***8 - Visualize : on `kubesonde.jackops.dev` upload `result.json` we get graphs !!***